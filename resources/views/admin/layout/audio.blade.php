@extends('index')
@section('title')
    Admin dashboard
@endsection

@section('content')
<div class="row ">
    <div class="col-lg-12 col-md-12 col-12">
        <!-- Page header -->
        <div class="border-bottom pb-3 d-lg-flex align-items-center justify-content-between">
            <div class="mb-2 mb-lg-0">
                <h1 class="mb-0 h2 fw-bold">All Audio </h1>
                <!-- Breadcrumb -->
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="{{ route('dashboard.url') }}">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="javascript:void()">Audio</a>
                        </li>
                    </ol>
                </nav>
            </div>
        </div>
    </div>
</div>

<div class="py-3">
    <div class="row">
        <div class="col-md-12 col-12 mb-5">
            <div class="d-flex justify-content-end pb-2">
                <a href="#" class="btn btn-dark btn-sm btn-click-modal" data-bs-toggle="tooltip" data-bs-placement="left" title="Add to album"><i class="fas fa-plus"></i></a>
            </div>
            <div class="show-content" style="display:none">
                <form action="{{ route('get.audio.add') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="">Title</label>
                            <input type="text" name="title" class="form-control" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="">audioUrl</label>
                            <input type="text" name="audioUrl" class="form-control" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="">imageUrl</label>
                            <input type="file" name="imageUrl" class="form-control">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="">Duration</label>
                            <input type="text" name="duration" class="form-control" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="">playCount</label>
                            <input type="number" name="playCount" class="form-control" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="">Album</label>
                            <select name="album_id" class="form-control">
                                @foreach($album as $albums)
                                    <option value="{{ $albums->id }}">{{ $albums->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="">Artist</label>
                            <select name="artist_id" class="form-control">
                                @foreach($artist as $artists)
                                    <option value="{{ $artists->id }}">{{ $artists->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <button type="submit" class="btn btn-primary">Save</button>
                        </div>
                    </div>
                </form>
            </div>
            <div class="card mt-2">
                <div class="card-body pt-2">
                    <div id="dataTableBasic_wrapper" class="no-footer">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="reponsive">
                                    <table id="dataTableBasic" class="table dataTable no-footer dtr-inline"
                                    style="width: 100%;" aria-describedby="dataTableBasic_info">
                                    <thead class="table-light">
                                        <tr>
                                            <th>#</th>
                                            <th>Title</th>
                                            <th>audioUrl</th>
                                            <th>imageUrl</th>
                                            <th>Duration</th>
                                            <th>playCount</th>
                                            <th>Album</th>
                                            <th>Artist</th>
                                            <th>Created Date</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($audio as $key => $audios)
                                            <tr class="odd">
                                                <td class="dtr-control">{{ $key + 1 }}</td>
                                                <td><?= $audios->title ?></td>
                                                <td><?= $audios->audioUrl ?></td>
                                                <td><?= $audios->audioUrl ?></td>
                                                <td><?= $audios->duration ?></td>
                                                <td><?= $audios->playCount ?></td>
                                                <td>
                                                    <?php
                                                        $albumss = \DB::table('album')->where('id',$audios->album_id)->first();
                                                        print $albumss->name
                                                    ?>    
                                                </td>
                                                <td>
                                                    <?php
                                                        $artistss = \DB::table('artist')->where('id',$audios->artist_id)->first();
                                                        print $artistss->name
                                                    ?> 
                                                </td>
                                                <td><?= date('d/m/Y',strtotime($audios->created_at)) ?></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<script>
    $(document).ready(function(){
        $('body').on('click','.btn-click-modal', function(){
            $('.show-content').toggle();
        });
    });
</script>
@endsection